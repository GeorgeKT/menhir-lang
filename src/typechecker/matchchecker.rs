        ...
        if let Pattern::Any(ref span) = c.pattern {
            if idx != m.cases.len() - 1 {
                return type_error_result(
                    span,
                    "A pattern match with _ must always be the last one in a match statement",
                );
            } else {
                any_match_seen = true;
            }
    ...
    for c in &m.cases {
        match c.pattern {
            Pattern::EmptyArray(_) => {
                if empty_array_seen {
                    return type_error_result(&c.span, "Duplicate pattern match, pattern match for [] already exists");
                } else {
...
fn check_sum_match_is_exhaustive<ST: SumTypeCaseIndexOf>(
    m: &MatchExpression,
    st: &ST,
    any_match_seen: bool,
) -> CompileResult<()> {
    ...
    let add_to_indices = |idx: Option<usize>, name: &str, indexes: &mut HashSet<usize>| {
        ...
        if indexes.contains(&idx) {
            type_error_result(&m.span, format!("Duplicate pattern match for {}", name))
        } else {
            indexes.insert(idx);
            Ok(())
        }
...
fn check_bool_match_is_exhaustive(m: &MatchExpression) -> CompileResult<()> {
        ...
        if let Pattern::Literal(Literal::Bool(_, v)) = c.pattern {
            if v {
                if true_seen {
                    return type_error_result(
                        &c.span,
                        "Duplicate pattern match, pattern match for true already exists",
                    );
                } else {
                    true_seen = true;
                }
                ...
                return type_error_result(
                    &c.span,
                    "Duplicate pattern match, pattern match for false already exists",
                );
            } else {
                false_seen = true;
            }
    ...
    if !true_seen || !false_seen {
        type_error_result(
            &m.span,
            "Incomplete pattern match, not all boolean values are matched against",
        )
    } else {
        Ok(())
    }
...
fn check_optional_match_is_exhaustive(m: &MatchExpression) -> CompileResult<()> {
    ...
    if !optional_seen || !nil_seen {
        type_error_result(
            &m.span,
            "Incomplete pattern match, not all possible optionals are matched again",
        )
    } else {
        Ok(())
    }
...
pub fn check_match_is_exhaustive(m: &MatchExpression, target_type: &Type) -> CompileResult<()> {
    ...
    match *target_type {
        Type::Array(_) | Type::Slice(_) => check_array_match_is_exhaustive(m, any_match_seen),

        Type::Sum(ref st) => check_sum_match_is_exhaustive(m, st.deref(), any_match_seen),

        Type::Enum(ref et) => check_sum_match_is_exhaustive(m, et.deref(), any_match_seen),

        Type::Struct(_) => {
            if !m.cases.is_empty() {
                type_error_result(
                    &m.span,
                    "Duplicate pattern match, structs can only have one pattern match",
                )
            } else {
                Ok(())
            }
        ...
        _ => {
            if !any_match_seen {
                type_error_result(&m.span, format!("Incomplete pattern match for type {}", target_type))
            } else {
                Ok(())
            }
        }
