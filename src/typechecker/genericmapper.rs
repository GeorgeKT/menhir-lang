...
mod tests
{
    ...
    fn test_slice()
    {
        ...
        assert!(make_concrete_type(&tm, &generic_type("a")) == Type::Int);
    }
    ...
    fn test_func()
    {
        ...
        assert!(r == Ok(aa));
        assert!(make_concrete_type(&tm, &generic_type("a")) == Type::Int);
        assert!(make_concrete_type(&tm, &generic_type("b")) == Type::Float);
        assert!(make_concrete_type(&tm, &generic_type("c")) == Type::Bool);
        assert!(make_concrete_type(&tm, &generic_type("d")) == string_type());
    }
