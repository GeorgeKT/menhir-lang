...
pub fn fill_in_generics(
    ...
    let new_generic = make_concrete(known_types, generic, span)?;
    ...
    match (&new_generic, actual) {
        ...
        (Type::Func(generic_ft), Type::Func(actual_ft)) => {
            ...
            for (ga, aa) in generic_ft.args.iter().zip(actual_ft.args.iter()) {
                let na = fill_in_generics(&aa.typ, &ga.typ, known_types, span)?;
        ...
        (Type::Struct(generic_st), Type::Struct(actual_st)) => {
            ...
            {
                let nt = fill_in_generics(ai, gi, known_types, span)?;
                implements.push(nt);
            }
        ...
        (Type::Sum(generic_st), Type::Sum(actual_st)) => {
            ...
            for (ga, aa) in generic_st.cases.iter().zip(actual_st.cases.iter()) {
                ...
                if let (Some(aat), Some(gat)) = (&aa.typ, &ga.typ) {
                    let nt = fill_in_generics(aat, gat, known_types, span)?;
        ...
        (Type::Result(generic_rt), Type::Result(actual_rt)) => {
            let ok_typ = if generic_rt.ok_typ.is_generic() {
                fill_in_generics(&actual_rt.ok_typ, &generic_rt.ok_typ, known_types, span)?
            } else {
                generic_rt.ok_typ.clone()
            };

            let err_typ = if generic_rt.err_typ.is_generic() {
                fill_in_generics(&actual_rt.err_typ, &generic_rt.err_typ, known_types, span)?
            } else {
...
mod tests {
    ...
    fn test_simple() {
        ...
        assert!(
            fill_in_generics(&Type::Int(IntSize::I32), &ga, &mut tm, &Span::default()) == Ok(Type::Int(IntSize::I32))
        );
        assert!(make_concrete(&tm, &ga, &Span::default()).unwrap() == Type::Int(IntSize::I32));
    }
    ...
    fn test_slice() {
        ...
        let r = fill_in_generics(&slice_type(Type::Int(IntSize::I32)), &ga, &mut tm, &Span::default());
        println!("tm: {:?}", tm);
        println!("r: {:?}", r);
        assert!(r == Ok(slice_type(Type::Int(IntSize::I32))));
        assert!(make_concrete(&tm, &generic_type("a"), &Span::default()).unwrap() == Type::Int(IntSize::I32));
    }
    ...
    fn test_array() {
        ...
        let r = fill_in_generics(&array_type(Type::Int(IntSize::I32), 10), &ga, &mut tm, &Span::default());
        println!("tm: {:?}", tm);
        println!("r: {:?}", r);
    ...
    fn test_func() {
        ...
        assert!(r == Ok(aa));
        assert!(make_concrete(&tm, &generic_type("a"), &Span::default()).unwrap() == Type::Int(IntSize::I32));
        assert!(make_concrete(&tm, &generic_type("b"), &Span::default()).unwrap() == Type::Float(FloatSize::F64));
        assert!(make_concrete(&tm, &generic_type("c"), &Span::default()).unwrap() == Type::Bool);
        assert!(make_concrete(&tm, &generic_type("d"), &Span::default()).unwrap() == Type::String);
    }
    ...
    fn test_simple_complex_mix() {
        ...
        println!("r: {:?}", r);
        assert!(r == Ok(array_type(Type::Int(IntSize::I32), 10)));
        assert!(make_concrete(&tm, &ga, &Span::default()).unwrap() == array_type(Type::Int(IntSize::I32), 10));
    }
